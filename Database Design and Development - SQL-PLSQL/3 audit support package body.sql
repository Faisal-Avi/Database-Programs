CREATE OR REPLACE PACKAGE audit_supp
IS
...
CREATE OR REPLACE PACKAGE BODY LPG_TEST.audit_supp
IS
    PROCEDURE ins_gbl_audit(
        in_audit_action         IN      VARCHAR2, 
        in_audit_table          IN      VARCHAR2,
    ...
    BEGIN
        IF (in_audit_action = 'U' AND NVL(in_old_value,'~~~~~') <> NVL(in_new_value,'~~~~~')) OR in_audit_action='D' THEN
            INSERT INTO gbl_audit_dtl(audit_id, audit_column, old_value, new_value)
            VALUES(in_audit_id,in_audit_column,in_old_value,in_new_value);
        END IF;
